 ...
 * 从 Milvus 中检索相关的日记条目
 */
async function retrieveRelevantDairies(question, k = 2) {
  try {
    // 生成问题的向量
 ...
 * 使用 RAG 回答关于日记的问题
 */
async function answerDailyQuestion(question, k = 2) {
  try {
    ...
    // 1. 检索相关日记
    const retrievedDailies = await retrieveRelevantDairies(question, k);
    if (retrievedDailies.length === 0) {
      console.log("未找到相关日记内容");
      return "抱歉，我没有找到相关的日记内容。";
    }

    // 2. 打印检索到的日记及相似度
    retrievedDailies.forEach((diary, i) => {
    ...
    // 3. 构建上下文
    const context = retrievedDailies
      .map((diary, i) => {
        return `[日记 ${i + 1}]
日期: ${diary.date}
心情: ${diary.mood}
...
async function main() {
  try {
    ...
    await answerDailyQuestion("我最近做了什么让我感到快乐的事情？", 2);
  } catch (error) {
