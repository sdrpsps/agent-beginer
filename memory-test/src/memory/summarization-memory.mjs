...
async function summarizationMemoryDemo() {
  ...
  if (allMessages.length >= maxMessages) {
    ...
    const summarizeMessages = allMessages.slice(0, -keepRecent);

    console.log("\n💡 历史消息过多，开始总结...");
    console.log(`📝 将被总结的消息数量: ${summarizeMessages.length}`);
    console.log(`📝 将被保留的消息数量: ${recentMessages.length}`);

    // 总结要被丢弃的信息
    const summary = await summarizeHistory(summarizeMessages);
