...
const readFileTool = tool(
  async ({ filePath }) => {
    try {
      ...
      console.log(
        `  [工具调用]: read_file("${filePath}") - 成功读取 ${(await content).length} 字节`,
      );
      return `文件内容:\n${content}`;
    } catch (error) {
      console.error(
        `  [工具调用]: read_file("${filePath}") - 读取失败: ${error.message}`,
      );
...
const writeFileTool = tool(
  async ({ filePath, content }) => {
    try {
      ...
      console.log(
        `  [工具调用]: write_file("${filePath}") - 成功写入 ${(await content).length} 字节`,
      );
...
const listDirectoryTool = tool(
  async ({ directoryPath }) => {
    try {
      ...
      console.error(
        `  [工具调用]: list_directory("${directoryPath}") - 读取失败: ${error.message}`,
      );
  ...
  },
  {
    ...
    schema: z.object({
      directoryPath: z.string().optional().describe("目录路径"),
    }),
  },
);
